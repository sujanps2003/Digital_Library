...
int main()
{
	printf("\n\t\t\t\t\t\t\t\xcd\xcd\xcd\xcd WELCOME \xcd\xcd\xcd\xcd\n\n");
	nd *head = NULL;
	int ch;
	while (1)
	{
		...
		case 2:
			...
			break;
		default:
			printf("\n\xcd\xcd\xcd\xcd END OF PROCESS \xcd\xcd\xcd\xcd\n\n");
			exit(0);
		}
	}
}
...
int verify_cust_id(int n)
{
	if (n == 1)
	{
		...
		while (x >= 1)
		{
			...
			fclose(f);
			x--;
			printf("\n\xcd\xcd\xcd\xcd Invalid Credentials...Attempts left: %d \xcd\xcd\xcd\xcd\n", x);
		}
...
void existing_cust() // EXISTING CUSTOMER.
{
	...
	{
		int c, i = 0;
		printf("\n\xcd\xcd\xcd\xcd 1.Books_Available\t2.Book_Borrow\t3.Outstanding_fine\t4.Book_Borrowed_List\t5.Return_Back \xcd\xcd\xcd\xcd\n\n");
		scanf("%d", &c);
		switch (c)
		{
		case 1:
	...
	while (feof(f) == 0)
	{
		...
		if (strcmp(s, b) == 0)
		{
			...
			if (m > 9)
			{
				...
				if ((update_user_data(by.user_name)) == 1)
				{
...
int update_user_data(char *s)
{
	...
	while (feof(f) == 0)
	{
		...
		if (strcmp(p, s) == 0)
		{
			...
			if (m == 9)
			{
				printf("\xcd\xcd\xcd\xcd REACHED THE MAXIMUM BOOKS \xcd\xcd\xcd\xcd\n\n");
				return 2;
			}
			m = m + 1;
			fseek(f, x, SEEK_SET);
...
void book_borrowed_list(char *s)
{
	FILE *f = fopen("Borrow.txt", "r");
	if (feof(f) != 0)
	{
		printf("\xcd\xcd\xcd\xcd NO RECORD AVAILABLE \xcd\xcd\xcd\xcd\n\n");
		return;
	}
	int i = 0;
	while (feof(f) == 0)
	{
		...
		{
			char *q = NULL, *r = NULL;
			q = (char *)malloc(sizeof(char) * 21);
			r = (char *)malloc(sizeof(char) * 21);
			fscanf(f, "%s%s", q, r);
			printf("Book_Name:  %s\nDate_of_Borrowing:  %s\n\n", q, r);
			free(q);
			free(r);
	...
	if (i == 0)
	{
		printf("\xcd\xcd\xcd\xcd NO BOOK BORROWED \xcd\xcd\xcd\xcd\n\n");
	}
	fclose(f);
}
...
void librarian()
{
	...
	while (j)
	{
		...
		if (strcmp(p, pass) == 0)
		{
			printf("\n\xcd\xcd\xcd\xcd VERIFICATION COMPLETED \xcd\xcd\xcd\xcd\n\n");
			printf("Press any key to proceed....");
			getch();
			system("cls");
			break;
	...
	while (1)
	{
		printf("\xcd\xcd\xcd\xcd 1.Enter Book in record\t2.Borrowing History\t3.Fine System\t4.Customers\t5.New Customers\t   6.Return_Back \xcd\xcd\xcd\xcd\n");
		scanf("%d", &c);
		switch (c)
		{
		case 1:
...
void borrow_history() // BORROW HISTORY.
{
	...
	if (feof(f) != 0)
	{
		printf("\n\xcd\xcd\xcd\xcd NO BOOKS AVAILABLE \xcd\xcd\xcd\xcd\n\n");
		return;
	}
	while (feof(f) == 0)
	{
...
void fine_history(FILE *f) // FINE HISTORY.
{
	...
	if (feof(f) != 0)
	{
		printf("\xcd\xcd\xcd\xcd NO FINE HISTORY AVAILABLE \xcd\xcd\xcd\xcd\n\n");
	}
	while (feof(f) == 0)
	{
		char *p = (char *)malloc(sizeof(char) * 21);
...
void add_fine(FILE *f) // ADDS FINE
{
